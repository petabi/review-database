...
impl From<RepeatedHttpSessionsV0_39> for crate::event::RepeatedHttpSessions {
    fn from(old: RepeatedHttpSessionsV0_39) -> Self {
        Self {
            ...
            dst_port: old.dst_port,
            proto: old.proto,
            start_time: old.time, // use same time for start_time since old structure doesn't have it
            end_time: old.time,   // use same time for end_time since old structure doesn't have it
...
impl From<PortScanV0_39> for crate::event::PortScan {
    fn from(old: PortScanV0_39) -> Self {
        Self {
            time: old.time,
            sensor: String::new(), // default empty sensor for migrated events
            src_addr: old.src_addr,
            dst_addr: old.dst_addr,
