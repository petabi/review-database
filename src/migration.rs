...
const DATABASE_VERSION_REQ: &str = ">=0.3.0,<=0.4.0-alpha.1";
...
pub fn migrate_data_dir<P: AsRef<Path>>(data_dir: P, backup_dir: P) -> Result<()> {
    ...
    let migration = vec![(
        VersionReq::parse(">=0.2,<=0.4.0-alpha.1").expect("valid version requirement"),
        Version::parse("0.4.0-alpha.1").expect("valid version"),
        migrate_0_2_to_0_3,
    )];
