    ...
    for (_key, old_value) in node_raw.iter_forward()? {
        ...
        if let Some(ref config) = old_inner_node.giganto {
            let remote = Remote {
                ...
                kind: crate::AgentRemoteKind::Datalake,
...
fn migrate_0_29_node(store: &super::Store) -> Result<()> {
    ...
    use bincode::Options;
    use chrono::{DateTime, Utc};
    use migration_structures::{Giganto, OldInnerFromV29BeforeV34, OldNodeFromV29BeforeV34};
    ...
    impl TryFrom<OldNodeBeforeV29> for OldNodeFromV29BeforeV34 {
        ...
        fn try_from(input: OldNodeBeforeV29) -> Result<Self, Self::Error> {
            ...
            let status = crate::AgentRemoteStatus::Enabled;
            if let Some(s) = input.settings.as_ref() {
                if s.hog {
                    ...
                    let agent = Agent {
                        node: input.id,
                        key: "hog".to_string(),
                        kind: crate::AgentRemoteKind::SemiSupervised,
                        config: Some(config),
                        draft: None,
                        status,
                    };
                ...
                if s.piglet {
                    ...
                    let agent = Agent {
                        ...
                        kind: crate::AgentRemoteKind::Sensor,
                        config: Some(config),
                        draft: None,
                        status,
                    };
                ...
                if s.reconverge {
                    ...
                    let agent = Agent {
                        ...
                        kind: crate::AgentRemoteKind::Unsupervised,
                        config: Some(config),
                        draft: None,
                        status,
                    };
            ...
            if let Some(s) = input.settings_draft.as_ref() {
                ...
                if s.giganto {
                    ...
                    giganto = Some(Giganto { status, draft });
                }
            }
            Ok(Self {
                id: input.id,
