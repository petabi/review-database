...
use crate::{Agent, AgentStatus, ConnectionlessAgent, Indexed, IterableMap};
...
/// ```
const COMPATIBLE_VERSION_REQ: &str = ">=0.35.0-alpha.1,<0.35.0-alpha.2";
...
pub fn migrate_data_dir<P: AsRef<Path>>(data_dir: P, backup_dir: P) -> Result<()> {
    ...
    let migration: Vec<Migration> = vec![
        ...
        (
            VersionReq::parse(">=0.34.0,<0.35.0-alpha.1")?,
            Version::parse("0.35.0-alpha.1")?,
            migrate_0_34_to_0_35_0,
        ),
    ];
...
fn migrate_0_34_to_0_35_0(store: &super::Store) -> Result<()> {
    migrate_0_35_node(store)
}

fn migrate_0_35_node(store: &super::Store) -> Result<()> {
    use bincode::Options;
    use migration_structures::OldInnerFromV29BeforeV34;

    use crate::{tables::InnerNode, IterableMap};

    let map = store.node_map();
    let node_raw = map.raw();
    for (_key, old_value) in node_raw.iter_forward()? {
        let old_inner_node = bincode::DefaultOptions::new()
            .deserialize::<OldInnerFromV29BeforeV34>(&old_value)
...
fn migrate_0_29_node(store: &super::Store) -> Result<()> {
    ...
    use bincode::Options;
    use chrono::{DateTime, Utc};
    use migration_structures::{OldInnerFromV29BeforeV34, OldNodeFromV29BeforeV34};

    use crate::{
        tables::{UniqueKey, Value},
        {collections::Indexed, IterableMap, NodeProfile},
    };
    ...
    impl TryFrom<OldNodeBeforeV29> for OldNodeFromV29BeforeV34 {
        type Error = anyhow::Error;
        ...
        fn try_from(input: OldNodeBeforeV29) -> Result<Self, Self::Error> {
            ...
            if let Some(s) = input.settings.as_ref() {
                ...
                if s.giganto {
                    giganto = Some(ConnectionlessAgent {
    ...
    for (_key, old_value) in node_raw.iter_forward()? {
        let old_node = bincode::DefaultOptions::new()
            ...
            .context("Failed to migrate node database: invalid node value")?;
        match TryInto::<OldNodeFromV29BeforeV34>::try_into(old_node) {
            Ok(new_node) => {
                for agent in &new_node.agents {
                    agent_raw.insert(agent.unique_key().as_ref(), agent.value().as_ref())?;
                }
                let inner: OldInnerFromV29BeforeV34 = new_node.into();
...
mod tests {
    ...
    fn migrate_0_26_to_0_29_node() {
        ...
        use crate::{
            collections::Indexed,
            migration::migration_structures::{
                OldInnerFromV29BeforeV34, OldNodeFromV29BeforeV34, PigletConfig,
            },
            types::FromKeyValue,
            Agent, Indexable,
        };
        ...
        let new_inner_node: OldInnerFromV29BeforeV34 = bincode::DefaultOptions::new()
        ...
        let new_node = OldNodeFromV29BeforeV34 {
    ...
    fn migrate_0_30_to_0_34_node() {
        ...
        use super::migration_structures::{
            DumpItem, OldInnerFromV29BeforeV34, OldNodeFromV29BeforeV34,
        };
        use crate::{
            collections::Indexed,
            migration::migration_structures::{GigantoConfig, HogConfig, PigletConfig},
            tables::{UniqueKey, Value},
            Agent, AgentKind, AgentStatus, ConnectionlessAgent, NodeProfile,
        };
        ...
        let old_node = OldNodeFromV29BeforeV34 {
            id: 0,
            name: "name".to_string(),
            name_draft: None,
            profile: None,
        ...
        let old_inner_node: OldInnerFromV29BeforeV34 = old_node.clone().into();
        ...
        assert!(super::migrate_0_35_node(&settings.store).is_ok());
        ...
        assert!(piglet.dump_items.is_some());
        assert!(piglet.dump_http_content_types.is_some_and(|v| v.is_empty()));
        assert!(new_node.ti_container.is_none());
    }
}
