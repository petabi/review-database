...
use attrievent::attribute::{HttpAttr, RawEventKind};
...
macro_rules! http_target_attr {
    ($event: expr, $raw_event_attr: expr) => {
        if let RawEventKind::Http(attr) = $raw_event_attr {
            match attr {
...
impl Match for HttpThreat {
    ...
    fn target_attribute(&self, raw_event_attr: RawEventKind) -> Option<AttrValue> {
        http_target_attr!(self, raw_event_attr)
    }
