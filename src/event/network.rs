...
use attrievent::attribute::{NetworkAttr, RawEventKind};
...
macro_rules! network_target_attr {
    ($event: expr, $raw_event_attr: expr) => {
        if let RawEventKind::Network(attr) = $raw_event_attr {
...
impl Match for NetworkThreat {
    ...
    fn target_attribute(&self, raw_event_attr: RawEventKind) -> Option<AttrValue> {
        network_target_attr!(self, raw_event_attr)
    }
}
