    ...
    fn score_by_packet_attr(&self, packet_attr: &[PacketAttr]) -> f64 {
        ...
        for attr in packet_attr {
            let Ok(raw_event_attr) =
                RawEventKind::from_type_and_attr_name(&attr.raw_event_type, &attr.attr_name)
            else {
                continue;
            };
            let Some(target_attr_val) = self.target_attribute(raw_event_attr) else {
                continue;
            };
...
mod tests {
    ...
    fn compare_attribute() {
        ...
        use attrievent::attribute::{DhcpAttr, DnsAttr, HttpAttr, RawEventType};
        ...
        let success_packet_attr = vec![
            PacketAttr {
                raw_event_type: RawEventType::Http.to_string(),
        ...
        let success_packet_attr = vec![
            PacketAttr {
                raw_event_type: RawEventType::Dns.to_string(),
        ...
        let success_packet_attr = vec![PacketAttr {
            raw_event_type: RawEventType::Dhcp.to_string(),
