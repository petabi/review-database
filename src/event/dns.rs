...
macro_rules! dns_target_attr {
    ($event: expr, $proto_attr: expr) => {
        match $proto_attr {
            ...
            DnsAttr::Query => AttrValue::String($event.query.clone()),
            DnsAttr::Answers => AttrValue::VecString($event.answer.clone()),
...
impl Match<DnsAttr> for DnsCovertChannel {
    ...
    fn target_attribute(&self, proto_attr: DnsAttr) -> Option<AttrValue> {
        Some(dns_target_attr!(self, proto_attr))
    }
}
