...
use attrievent::attribute::{RawEventKind, SshAttr};
...
macro_rules! ssh_target_attr {
    ($event: expr, $raw_event_attr: expr) => {{
        if let RawEventKind::Ssh(attr) = $raw_event_attr {
...
impl fmt::Display for BlockListSshFields {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "sensor={:?} src_addr={:?} src_port={:?} dst_addr={:?} dst_port={:?} proto={:?} last_time={:?} client={:?} server={:?} cipher_alg={:?} mac_alg={:?} compression_alg={:?} kex_alg={:?} host_key_alg={:?} hassh_algorithms={:?} hassh={:?} hassh_server_algorithms={:?} hassh_server={:?} client_shka={:?} server_shka={:?}",
            self.sensor,
...
impl Match for BlockListSsh {
    ...
    fn target_attribute(&self, raw_event_attr: RawEventKind) -> Option<AttrValue> {
        ssh_target_attr!(self, raw_event_attr)
    }
}
